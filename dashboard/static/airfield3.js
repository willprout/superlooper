/* Superlooper airfield v3 — hub terminal, landmark tower, banner tags, fleet.
   Lifted verbatim from design/project/airfield3.js (the pixel-perfect baseline); the ONLY
   changes are marked "LIVE-FIELD EXTENSION": an opts.noDemoAircraft flag (the live field draws
   only REAL flights, never the demo's parked pair or taxiing plane), drawCrest folded in from
   airfield.js, and extra live{} exports the animation engine binds. With no extension opts set,
   output is pixel-identical to the prototype (proven by the compare harness screenshot). */
(function () {
  'use strict';
  ...
  function drawOverview(canvas, opts) {
    ...
    // (LIVE-FIELD EXTENSION: skipped when opts.noDemoAircraft — the live engine draws only REAL flights)
    if (!opts.noDemoAircraft) {
    ...
    if (time !== 'day') {
      ...
      if (!opts.noDemoAircraft) navLights(c, 130, 139, 'W', true);
    }

    // SL-23 taxiing in (kept above lighting pass for readability)
    // (LIVE-FIELD EXTENSION: also demo-only — a truthful field draws no invented flights)
    if (!opts.noDemoAircraft) plane(c, 130, 139, 'W', {});
  ...
  // LIVE-FIELD EXTENSION: folded in verbatim from airfield.js drawCrest (the only survivor of
  // that file per the build plan); opts.bg takes the airline's tail color.
  ...
  window.Airfield3 = {
    ...
    drawCrest: drawCrest,
    // LIVE-FIELD EXTENSION: everything the N-flight engine binds (sprites + ground props); the
    // engine composes these — it never invents pixels of its own vocabulary.
    live: { plane: plane, planeSmall: planeSmall, planeAwacs: planeAwacs, navLights: navLights,
            banner: banner, contrail: contrail, tiedown: tiedown, windsock: windsock,
            glow: glow, PAL: PAL, shade: shade }
  };
})();
