...
# =============================== the session fact (issue #310) ===============================

def test_build_flight_carries_whether_a_session_window_was_recorded():
    # Issue #310: the flight card's Open-session-window button needs ONE fact — did the loop record
    # a session window for this lane? That fact is the engine's own pane marker (state/panes/<id>),
    # the SAME thing `superlooper tidy` selects on, so the button and the closer can never disagree
    # about which lanes have a window.
    #
...
def test_build_flight_session_window_fails_closed_when_the_fact_is_missing():
    ...
    # window, never a hopeful one. A button that addresses a name nothing vouches for is worse than
    # no button.
