...
def _route_janitor(clean, body_bytes, janitor):
    """The Janitor endpoints (issue #121) — the GitHub-side debris sweep, the third LOCAL COMMAND
    button (same class as Tidy). Same-origin is already enforced by the caller (a foreign page must
    not be able to sweep GitHub any more than it could drive the label writer). ``janitor=None`` (a
...
def route(method, path, snapshot_provider, static_root, *, actions=None, body=b"", origin=None,
          host=None, desk=None, tidy=None, janitor=None, replay_provider=None,
...
def make_handler(snapshot_provider, static_root, actions=None, desk=None, tidy=None, janitor=None,
    ...
    enables the Tidy local-command endpoints (issue #41); ``janitor`` (a ``lib.janitor.Janitor``)
    enables the Janitor GitHub-sweep endpoints (issue #121); ``replay_provider`` / ``digest_provider``
    ...
    class _Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            self._write(route(self.command, self.path, snapshot_provider, static_root,
                              ...
                              desk=desk, tidy=tidy, janitor=janitor))

    return _Handler


def build_server(snapshot_provider, static_root, port=8611, host=BIND_HOST, actions=None, desk=None,
                 tidy=None, janitor=None, replay_provider=None, digest_provider=None):
    ...
    9); ``tidy`` wires the Tidy local-command endpoints (issue #41); ``janitor`` wires the Janitor
    GitHub-sweep endpoints (issue #121); ``replay_provider`` / ``digest_provider`` wire the on-demand
    replay + digest GETs (Task 11); omit any for a surface that stays off. ``port=0`` binds an
    ephemeral port (tests). Call ``.serve_forever()`` to run it."""
    if host != BIND_HOST:
        raise ValueError(
    ...
    return ThreadingHTTPServer((host, port),
                               make_handler(snapshot_provider, static_root, actions, desk, tidy,
                                            janitor, replay_provider, digest_provider))
