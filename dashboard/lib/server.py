...
# The Open-session-window endpoint (issue #340) — the newest member of the local-command class and
# the only ONE-STEP one. It shells the engine's read-only `superlooper focus-session` to bring a
...
def _route_session_window(body_bytes, session_window):
    """The Open-session-window endpoint (issue #340) — a local-command verb. Same-origin is already
    ...
    bright line of this endpoint: the client names a FLIGHT, and ``lib.session_window.lane_id``
...
def route(method, path, snapshot_provider, static_root, *, actions=None, body=b"", origin=None,
    ...
    backs the Open-session-window endpoint (issue #340), which shells the engine's read-only
    ``superlooper focus-session`` to bring one flight's own window to the front."""
...
def make_handler(snapshot_provider, static_root, actions=None, desk=None, tidy=None, restart=None,
    ...
    ``lib.session_window.SessionWindow``) enables the Open-session-window endpoint (issue #340).
...
def build_server(snapshot_provider, static_root, port=8611, host=BIND_HOST, actions=None, desk=None,
    ...
    ``session_window`` wires the Open-session-window endpoint (issue #340); ``replay_provider``
