...
classifier's TUI patterns, what a session's own RECORD says about it, what a DELIVERED prompt looks
like, trust pre-acceptance, usage/quota reading, the cross-reviewer command
...
credential namespace from a config-dir string — issue #314; `usage.py` reads it rather than keeping
a second copy, so the two can never disagree about which account is being measured).
