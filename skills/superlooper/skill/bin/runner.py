...
# machine refused for hours while `claude auth status` answered `unknown`. No hold, no alert, no
# pause. #320's two streaks could see none of it: they count launches THIS process ran and key on
# ONE evidence reason reaching a cap. What the journal actually recorded that day was
#
#     08-25 19:03  watchdog      launch_failed  d26  rc=5   (the runner-env `gh`)
#     08-25 19:08  watchdog      launch_failed  d27  rc=5
#     08-25 19:13  watchdog      launch_failed  d28  rc=5
#     08-26 15:19  debug_launch  launch_failed  d29         ("CLAUDE IDENTITY REFUSED …")
#
# and not one launch of the runner's own: the queue was serialized behind a wildcard in-flight lane
# that never exited, so it attempted none for over a day. Any rule that needs a sample from the
# runner's own launches is silent on the outage it was written for.
#
# WHAT THE EVIDENCE IS. Two INDEPENDENT SPAWNERS refusing the same way. Each spawner runs the
# launcher in its own environment — the runner's, the watchdog's launchd job, an operator's shell
# or the dashboard — so one of them refusing says only that ITS environment is wrong, and two of
# them agreeing is the thing neither can explain away. That is what the incident produced (the
# watchdog's launchd job and the owner's Debug tap), and it is what rules out the two ways of being
# wrong about the same records:
#
#   * ONE SPAWNER, MANY FLIGHTS. The watchdog mints a FRESH d<N> for every retry of one episode
#     (three per episode), so a watchdog started with a bare PATH refuses every flight identically
#     while the runner's own launches are demonstrably fine. Counted as distinct evidence that
#     would freeze a healthy queue — and the page this hold raises is itself a watchdog signal, so
#     it would re-arm from the episode it opened.
#   * ONE SPAWNER, MANY IDS. `superlooper debug` and `superlooper resume` are shelled from the same
#     places (an operator's shell, the dashboard), and `resume` carries a LANE's own `i<N>`. One
#     shell with an ambient ANTHROPIC_API_KEY produces two ids and two verbs; it is still one
#     environment, so it is one spawner here.
#
# DERIVED, NOT ACCUMULATED, and that is load-bearing. An in-memory streak has to be reconciled with
# the journal for the launches this process did not run, and every incremental reconciliation this
# issue tried (a byte offset, a timestamp watermark, a lagged one) had a boundary at which a
# VERIFIED DELIVERY fell outside the window while the refusals it answered fell inside — which
# re-holds a machine that just launched a session. A pure function of the journal's tail has no
# such boundary, costs ~2 ms, and reconstructs itself exactly across a restart.
SPAWNER_RUNNER = "runner"
SPAWNER_WATCHDOG = "watchdog"
SPAWNER_OPERATOR = "operator"
# This runner's own launch outcomes. `recover` only at the `exited` tier — the only tier that
# relaunches; it reads "ok" for a delivered NUDGE too, and an i336 lane whose auth died in-process
# answers nudges all day while starting nothing.
RUNNER_LAUNCH_ACTS = ("launch", "resolve_conflict")
RUNNER_RELAUNCH_ACT = "recover"
RUNNER_RELAUNCH_TIERS = ("exited",)
RUNNER_DELIVERY_OK = "ok"
# The FOREIGN spawners: {act: (success outcome, failure outcome, spawner)}. A table and not a
# branch, so WHEN A FOURTH SPAWNER APPEARS ITS ROW BELONGS HERE — with the environment it runs in,
# which is what the two-spawner rule actually counts. The triage flight (#448) ships disabled and
# journals no launch act yet; `actions._is_session_id` already admits its `t<N>` ids.
FOREIGN_LAUNCH_OUTCOMES = {
    "watchdog": ("launched", "launch_failed", SPAWNER_WATCHDOG),
    "debug_launch": ("launched", "launch_failed", SPAWNER_OPERATOR),
    "resume": ("resumed", "resume_failed", SPAWNER_OPERATOR),
}
# NO TIME WINDOW, deliberately. The streak is "every refusal since the last verified delivery", and
# an earlier cut bounded it to six hours as well — which is a time-based EXPIRY wearing a window's
# name: while a hold stands the only thing that flies is the #115 canary, whose refusals are not
# samples, so nothing refreshes the streak and it aged below the cap mid-outage. decide reads that
# fall as recovery — it journals "launch delivery verified again" over a machine that verified
# nothing, retracts the alert, resumes, and walks the at-cap lanes into parks. The real incident
# settles it too: its two independent samples were TWENTY HOURS apart. The only bound is
# `journal.tail`'s byte slice, which is a bound on how much history is READ rather than a clock on
# what it means, and truncating a prefix is safe by construction — a clear that scrolls off takes
# every refusal it answered with it.
AUTH_DEATH_STREAK_REASONS = frozenset({
    ...
    "claude_identity_wrong",         # rc=7 — the SESSION's Anthropic account is absent/wrong
    "claude_identity_wrong_runner",  # rc=8 — the machine's fleet config dir is logged out
    "env_poisoned",                  # rc=6 — the launch-floor scrub could not clean the env
})
# WHICH refusals are evidence about the MACHINE — the set above. Not every failed launch is: a
# worktree that would not create, a brief that could not be written, a missing base branch name
# THIS issue's own state, and two of those in a row are two broken lanes; paging the owner "your
# Claude login has expired" over a stale git index.lock is the mis-blame #299/#320 exist to end.
# Nor is a dead cmux pane or an unfired shim: those are the DELIVERY CHANNEL, with detectors and
# remedies of their own. What is left is the credential/environment family — the launcher refused
# because the identity or the environment a session would run under is wrong. Each is per-issue on
# one sample (#320 keeps them so), and #320 escalates them only when the SAME one repeats across
# two distinct ISSUE lanes; the 2026-08-26 shape is what falls between.
...
# The rotation notice a re-approval posts (#177). Marked for the same reason as the two above: it
# is the RUNNER's bookkeeping addressed to the owner ("your re-approval retired branch X"), and an
# un-marked comment would land in every rebuilt brief under the binding-owner-amendment header —
# runner prose read to the worker as William's word.
REBUILD_MARKER = "<!-- superlooper-rebuild -->"
...
class Runner:
    def __init__(self, repo, config, state_home=None, pane=None, agent="claude",
        ...
        # This process's ENGINE GENERATION (issue #405) — minted in __init__ rather than in run() so
        # a tick driven directly (tests, a one-shot diagnostic) carries one too, and so it is fixed
        # for the whole process: every tick of this runner shares it, which is what makes a standing
        # hold re-announce exactly ONCE per generation rather than once per tick.
        self._hold_generation = self._mint_hold_generation()
    ...
    @staticmethod
    def _mint_hold_generation(now=None):
        """The ENGINE GENERATION token this process's ticks carry (issue #405).
        ...
        This token is the fix, and it works by RE-KEYING rather than reaping. It joins each hold's
        dedup key, so the first tick of a new process finds every standing stamp keyed to the
        previous generation, re-journals it once, and then goes quiet again: "once per engine
        generation" instead of "once per lifetime". A restart is exactly the moment an operator's
        model of the loop is rebuilt, and it is also when a republished engine takes effect.

        Re-keying rather than CLEARING the stamps at boot is load-bearing twice over:

          * the stamps and their reasons survive, so a boot that cannot re-derive its holds never
            blanks them. The launch-phase holds live behind `not launches_held` — and a sleeping
            display is normal overnight — so a 3am resurrection restart that cleared them would have
            left the 08:45 report saying "nothing is held" with the whole queue held. That is the
            exact silence this issue exists to end, reintroduced by its own fix;
          * nothing ever creates a clock without a stamp, so a hold's AGE cannot be orphaned and
            adopted by a later, unrelated episode. Stamp and clock are written together by the
    ...
    def disk_view(self, now):
        ...
        return {
            ...
            "frozen": frozen,
            "alert": alert,
            # This process's engine generation (#405) — joins each standing hold's journaling dedup
            # key, so every hold re-announces once per generation instead of once per lifetime.
            "hold_generation": self._hold_generation,
    ...
    def _settle_merged_lane(self, iid, exit_timeout=None):
        ...
        Under the prune knob this waits on a worker that is normally already dying, but it is a real
        bound: N lanes settling in one tick can cost N x WORKER_EXIT_TIMEOUT of tick latency. The
        default config has always paid exactly that on the same lanes, and the drain re-entry probes
        once rather than waiting, so no sweep pays it per lane.
        ...
        The declined-teardown marker (state/pending_teardown/<id>) is this method's to keep or drop,
        because under these knobs it no longer means one fixed thing. _drain_pending_teardowns retries
        a marker by tearing down, and for a merged lane it retries THROUGH HERE — a bare
        remove_worktree=True retry would prune a checkout `cleanup_merged_worktrees: false` asked to
        keep and close a window `auto_close_merged_windows: false` asked to keep, one tick after the
        settle, behind the operator's back and with no live pid left to refuse it (the #149 guarantee
        itself). So:

          window knob off   -> nothing is owed on this lane ever again: DROP any marker an earlier
                               decline left (a re-approval that could not clear the lane, the opt-in
                               parked reaper, a #190 reclaim refusal — that last one keeps its marker
        ...
        the window knob is off and nothing was attempted. No caller acts on it today."""
        marker = os.path.join(self.state, "pending_teardown", iid)
    ...
    def _teardown_session(self, iid, remove_worktree=False, exit_timeout=None, guard_worktree=False,
        ...
        `remove_worktree=False` is the D4 relaunch close: it prunes nothing, so (unless the caller
        asks for a wait — see `await_exit`) it has nothing to protect and does not wait, a relaunch
        must not pay a stall for a pid it is about to supersede. That path's behavior is byte-for-byte
        what it was before this function existed.

        `await_exit` can only ADD a wait to the one a prune already implies, and exists for the ONE
        caller that prunes nothing yet still needs the exit observed: the keep-the-checkout settle of
        a merged lane (_settle_merged_lane, #178). There, ending the session IS the action, and
        _close_pane is best-effort (rc ignored, a missing pane record a silent no-op), so without the
        wait a failed close would be recorded as a session ended while the markers and lock that are
        the only handles on the survivor were deleted. Note what a decline then means for that
        caller: no worktree is at stake, so no PRUNE is owed and none is recorded here — the settle
        records the owed CLOSE itself, because the marker's meaning is that caller's to choose.
        ...
        # OR, never a ternary: await_exit may only ADD a wait. `remove_worktree=True,
        # await_exit=False` must not be expressible — that is a prune with no liveness check, the
        # #149 violation itself, and a future caller should not be able to write it by accident.
        wait = remove_worktree or bool(await_exit)
    ...
    def _drain_pending_teardowns(self, st):
        ...
        for iid in pending:
            ...
            if status == "merged":
                # (#178) A merged lane's teardown is KNOB-GATED, so retry it through the same settle
                # that decided it. Retrying with a bare remove_worktree=True would prune a checkout
                # `cleanup_merged_worktrees: false` asked to keep and close a window
                # `auto_close_merged_windows: false` asked to keep — behind the operator's back, and
                # with no live pid left to refuse it once the settle cleared the lock. Under the
                # shipped defaults this is the same call as the branch below (a merged lane never
                # guards), so the merge-path cleanup this drain exists to retry is untouched.
                self._settle_merged_lane(iid, exit_timeout=0)
    ...
    def _launch_attempt_streak(self, now=None):
        ...
        delivery and adding on any classified refusal. The `ts` field is never used to order: this
        runner stamps its records with the clock the TICK started on, so a delivery that really came
        last can carry an earlier stamp than a refusal another process wrote meanwhile.

        `now` is accepted and unused — this streak has no clock at all (see the constants' note on
        why there is no window) — and kept so the publish site reads like the two beside it.
        ...
        for rec in records:
            ...
            if mine:
                if outcome == RUNNER_DELIVERY_OK:
                    ...
                    self._note_attempt_failure(samples, rec, SPAWNER_RUNNER)
                continue
            row = FOREIGN_LAUNCH_OUTCOMES.get(act)
            if row is None or not isinstance(outcome, str):
                continue                               # NB: an act with NO outcome must not match a
            ok, failed, spawner = row                  # missing table row and read as a delivery
            if outcome == ok:
    ...
    def _streak_reason(rec):
        ...
        on them (#152). A foreign spawner's carry what its verb recorded — the launcher's rc, its
        stderr, or both — and either alone is enough, because `lib/evidence` reads the TEXT first
        and falls back to the rc. That matters: the owner-tapped Debug record from the real
        2026-08-26 outage carries no rc at all, only the launcher's "CLAUDE IDENTITY REFUSED" line,
        and requiring an rc made the whole existing journal invisible to this detector.

        rc=1 is the stand-in when a record carries no rc: it is the launcher's own "aborted before
        any pane could host a worker", the code whose text pass IS the classification. A text that
        matches nothing then lands on a CHANNEL reason, which the family filter drops — so an
        unreadable refusal stays unreadable rather than borrowing a credential's remedy."""
        ev = rec.get("evidence")
        if isinstance(ev, dict) and isinstance(ev.get("reason"), str):
            return ev["reason"]
        rc = rec.get("rc")
        text = rec.get("error")
        if not isinstance(text, str) and (not isinstance(rc, int) or isinstance(rc, bool)):
            return None
        try:
            return evidence.build(
                "launch", rc if isinstance(rc, int) and not isinstance(rc, bool) else 1,
                text if isinstance(text, str) else "")["reason"]
        except Exception:                              # pragma: no cover - defensive
            return None
    ...
    def _exec_launch(self, a, now):
        ...
        self._update_issue(iid, {"branch": branch, "num": num, "type": p.get("type"),
                                 ...
                                 # ...and every hold's AGE clock, generation key and the dead-worker
                                 # flag with them (#405): a launched lane must not keep reporting a
                                 # hold that ended at the moment it started.
                                 "launch_hold_since": None, "wildcard_hold_since": None,
                                 "queue_invalid_since": None, "relaunch_held": False,
                                 "launch_hold_generation": None, "wildcard_hold_generation": None,
                                 "queue_invalid_generation": None})
    ...
    def _exec_park(self, a, now):
        ...
        def m(st):
            ...
            i["launch_hold_since"] = None       # ...and the age clock, generation key and
            i["launch_hold_generation"] = None  # dead-worker flag it owns (#405), so a park
            i["relaunch_held"] = False          # never reads as a standing hold
    ...
    def _exec_reapprove(self, a, now):
        ...
        def reset(st, i):
            ...
            # a name the conflict ladder already used. A lane with NO branch stamp was never even
            # handed to the launcher, so there is nothing to retire — leave the stamp None and let
            ...
            # cost of guessing wrong this way is a cosmetic `-r1`; the other way it is the bug back.
            meta["type"] = i.get("type")
            ...
            i.update({"status": "ready", "requeue_front": False, "recheck_failed": False,
                      ...
                      "queue_invalid_since": None, "relaunch_held": False,
                      "launch_hold_generation": None,  # ...and re-keyed, so the fresh episode's
                      "wildcard_hold_generation": None,   # own hold speaks under this generation
                      "queue_invalid_generation": None,
        ...
        # An INVESTIGATION is exempt: it opens no PR by contract, so the lookup is the pure waste
        # issue #21 already refuses elsewhere, and a branch-retirement notice about a branch that
        # carries no code is noise on the owner's issue. It still rotates — one invariant, no
        # per-type carve-out in the naming.
        labelled, failed = ((None, []) if not rotated or meta.get("type") == "investigate"
                            else self._retire_old_branch(num, rotated))
        parts = [f"reset {old or 'nothing'}"]
        if rotated:
            parts.append(f"rebuilding on {rotated['new']}")
        if labelled:
            parts.append(f"superseded PR #{labelled}")
        if failed:
            # NEVER report bookkeeping that did not land, and NAME what did not (fresh review P1 +
            # P2): nothing retries this — the state reset has already dropped `pr` and left
            # REAPPROVAL_STATUSES, so decide will not re-emit a completed reapprove, and every
            # downstream reconciliation (janitor close-pr, delete-branch) keys off the `superseded`
            # LABEL. A silent or misattributed failure orphans the retired PR forever behind a
            # journal line that says it was handled.
            parts.append("gh bookkeeping incomplete: " + ", ".join(failed))
    ...
    def _retire_old_branch(self, num, rotated):
        ...
        the janitor's `superseded` lane. Returns (pr_number_actually_labelled_or_None, [what failed]).
        ...
        Two duties, both owed to the owner:
          * the ISSUE comment always fires — a re-approval that silently retires a branch leaves the
            ...
            the one who decides whether to go get it. It carries REBUILD_MARKER so brief.build reads
            it as the runner's own bookkeeping, never as a binding owner amendment.
          * a PR still OPEN on that branch is now an orphan (no issue points at it, nothing will
            ...
            lands in the same lane as one reached by a conflict. The number comes back ONLY when the
            LABEL landed, because the label IS what the janitor can see.
        ...
        branch it must never touch). Its `--state all --limit 1` contract means a branch carrying
        BOTH a closed and an open PR can answer with the closed one; that under-labels (leaves the
        open PR for the owner) rather than mis-labelling, which is the safe side.

        Nothing here can fail the re-approval — the fresh branch is the safety property and this is
        bookkeeping — but every result travels back so the outcome names what really landed. It runs
        AFTER the state write deliberately: running it first would make it retryable through decide's
        re-emission, at the price of double-posting the notice on every partial failure, and a
        duplicated owner-facing comment is the worse of the two (the failure is at least named)."""
        read = gh.pr_for_branch(rotated["old"])
        failed, labelled = [], None
        if not read.ok:
            # A REFUSED read is NOT "no PR" (#61's refused != empty discipline). Reporting it as a
            # clean no-PR retirement would assert something we never looked at and hand back an
            # outcome nothing retries, while an open PR sat unlabelled — invisible to the janitor
            # forever. Say so in BOTH channels instead.
            failed.append(f"the PR on `{rotated['old']}` could not be read (refused), so nothing "
                          "was labelled `superseded`")
            tail = (" Its PR could not be read just now, so nothing was labelled `superseded` — "
                    "worth a look by hand.")
        else:
            pv = read.pr if isinstance(read.pr, dict) else {}
            n = pv.get("number") if pv.get("state") == "OPEN" else None
            pr = n if type(n) is int else None
            if pr:
                if gh.pr_add_labels(pr, ["superseded"]):
                    labelled = pr
                else:
                    failed.append(f"the `superseded` label on PR #{pr}")
                if not gh.pr_comment(pr, "Superseded by superlooper: this issue was re-approved and "
                                         "is being rebuilt from scratch on a fresh branch "
                                         f"(`{rotated['new']}`). Branch preserved; nothing "
                                         "auto-closed."):
                    failed.append(f"the supersede note on PR #{pr}")
                tail = (f" PR #{pr} on `{rotated['old']}` is superseded (branch preserved on the "
                        "remote, nothing auto-closed).")
            else:
                # True whether or not the branch was ever created: a launch that never delivered
                # leaves a stamp for a branch that exists nowhere, and "anything committed on it"
                # is then vacuously nothing.
                tail = (f" Anything committed on `{rotated['old']}` stays there — nothing is "
                        "auto-closed or deleted.")
        if not gh.comment(num, f"{REBUILD_MARKER}\nRe-approved — rebuilding from this issue on a "
                               f"fresh branch (`{rotated['new']}`), retiring `{rotated['old']}`."
                               f"{tail}"):
            failed.append(f"the retirement comment on issue #{num}")
    ...
    def _exec_recover(self, a, now):
        ...
        if tier == "exited":
            ...
            self._update_issue(iid, {"launch_hold_reason": None, "launch_hold_since": None,
                                     "launch_hold_generation": None, "relaunch_held": False})
            rc = self._run_script([self._script(LAUNCHER), iid],
    ...
    def _exec_wildcard_hold(self, a, now):
        ...
        self._update_issue(a["id"], {"wildcard_hold_journaled": True,
                                     "wildcard_hold_generation": a.get("generation")},
    ...
    def _exec_queue_invalid(self, a, now):
        ...
        self._update_issue(a["id"], {"queue_invalid_signature": a.get("signature"),
                                     "queue_invalid_generation": a.get("generation")},
    ...
    def _start_hold_clock(issue, key, now):
        ...
        "Only if not already running" is what makes the once-per-generation re-announce a no-op for
        the clock: the first tick of a new engine generation re-derives the same cause and re-stamps
        it under the new generation key, and this leaves the original start time alone. Without that,
        a hold could never be older than the last restart and the age alert could never fire on the
        multi-day stall it exists to catch.

        The clock is written by the same executors that write the stamp, and cleared by the same five
        sites (launch, recover, resolve, park, re-approve) — so a clock without a stamp cannot exist,
        and a later, unrelated hold episode can never inherit an orphaned age.

        A wrong-typed stored value (bool is an int — excluded) restarts the clock rather than being
        trusted; an unusable clock is worth less than an honest recent one."""
        cur = issue.get(key)
    ...
    def _exec_close_investigate(self, a, now):
        ...
        interview's verdict clean. That evidence is not quite the gating route's, and the difference
        cuts against this change rather than for it — `gating` additionally requires the report file
        on disk, which is what lets a lane reach the gate at all, while the reconciliation
        preconditions ask for no report. The route carrying LESS evidence is the one reaching into
        park-family territory. What settles it anyway is a fact rather than a reading: closing the
        owner's ISSUE on this route already ships and always has, so decide is already trusted to
        conclude a parked investigation unilaterally. Standing down the window the loop itself
        opened is the strictly smaller act. The lane stops being park-family in the same breath, so
        the merged knobs govern it rather than the opt-in parked reaper. _exec_absorb_close is the
        shape but NOT the authority — there the owner's close IS the owner verb, and it says so.
        Flagged for the owner rather than smuggled, and cheap to reverse if he rules the other way
        (read the prior status here and skip the settle for `parked`); carving it out pre-emptively
        would instead leave the identical leak #275 exists to close, one route along.
        ...
        land — the guard is _teardown_session's step 4, after the pane, markers and lock are gone).
        ...
        returns a REASON, so a future path could refuse on any reason CONTAINING `unpushed` (the
        compound "dirty+unpushed" is one of its four answers, and testing for equality would let
        through the very case being protected) plus `unreadable` — the genuinely unrecoverable
        shape, commits on no remote ref — while still pruning the untracked scratch that would make
        a blanket guard inert. Realizing it means touching the shared settle,
    ...
    def _exec_unfreeze(self, a, now):
        ...
        # the CLEAR predicate, which is deliberately WEAKER than gate.freeze_exempt, the predicate
        # that decides what may be MERGED under a freeze (#295). Do not collapse them: this one is
        # asked only while dev reads GREEN, so an unknown-owner marker resolving to "clear it" just
        # ends a freeze on a healthy mainline, whereas crossing one would merge under a freeze
        # nobody can vouch for. The set this clears is therefore a strict SUPERSET of the set the
        # standing-rule fix may cross, which is the safe direction (nothing is crossable-but-never-
        # clearable).
