#!/usr/bin/env bash
# Claude "PreToolUse" hook. Fires before EVERY tool call. Denies exactly three named hazards —
# AskUserQuestion, pattern-kills (pkill -f / killall), and a mechanically-invalid `gh issue create`
# (issue #225: no `type:` label / no parseable `## Loop metadata`, validated BEFORE the issue
# exists) — in the UNATTENDED sessions superlooper launches, and lets everything else proceed
# untouched. This script fences on SL_ISSUE_ID +
...
# ruling on #185, and to the issue-filing format by his #225 redesign): AskUserQuestion in an
# unattended lane (i280), a pattern-kill that matched the owner's own live dashboard, and an issue
# filed without the metadata that makes it launchable. lib/worker_pretooluse.py decides; the spike proved
