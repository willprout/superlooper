...
| `auto_close_merged_windows` | `true` | Auto-close a lane's cmux window (and, ordered by #149, reclaim its worktree) once it has **successfully merged and landed** — owner ruling 2026-07-16 (#168). This is the only #149-family auto-close of a merged lane. Set `false` to keep merged windows/worktrees for inspection (the pre-#149 "nothing auto-closed" posture); `superlooper tidy` then remains your explicit word to close the **window** (tidy closes windows only — it never prunes a worktree, so the merged checkout stays on disk for manual removal). |
