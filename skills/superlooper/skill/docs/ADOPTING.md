...
| `triage.home` | `"checkout"` | Where a triage flight runs. `checkout` opens the repo's **real checkout**, so the flight sees what an orchestrator sees — the gitignored working files a fresh worktree by definition cannot show. Its working tree stays read-only to the flight (its only writes are GitHub and its own state home). Set `worktree` if your gitignored overlay is sensitive, accepting that overlay-aware triage is lost there; that home is a **detached** checkout of `dev_branch`, since the flight creates no branch. |
