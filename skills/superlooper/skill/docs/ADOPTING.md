...
| `cleanup_merged_worktrees` | `true` | Remove a worktree after its issue merges. Strictly about the **checkout**: `false` keeps the merged worktree on disk for inspection, and nothing else — the session is still closed (that is `auto_close_merged_windows`, below). |
| `auto_close_merged_windows` | `true` | Auto-close a lane's cmux window (and, ordered by #149, reclaim its worktree unless `cleanup_merged_worktrees` says keep it) once it has **successfully merged and landed** — owner ruling 2026-07-16 (#168). This is the only #149-family auto-close of a merged lane. Set `false` to keep merged windows/worktrees for inspection (the pre-#149 "nothing auto-closed" posture); the worktree persists too, since a prune can never run under the live CLI it would leave open. `superlooper tidy` then remains your explicit word to close the **window** (tidy closes windows only — it never prunes a worktree, so the merged checkout stays on disk for manual removal). |
| `cleanup_parked_worktrees` | `false` | Reclaim the worktrees of park-family terminal issues (parked / needs-owner / bounced) on every tick (issue #41). **Off by default (owner ruling 2026-07-16, #168):** a park-family lane's window AND worktree persist until an owner verb (re-approve / drop / tidy) resolves the lane, so the owner can open stalled work and look at the session. Set `true` to opt back into the disk-bounding reaper on a disk-constrained machine, accepting that it closes park-family windows; the #190 dirty/unpushed refusal still guards every prune. |
...
| `notify.cmd` | `null` | A generic notify command template (`{title}`/`{body}`) if you don't use iMessage. `{title}` is the text's first line, `{body}` the rest. |
| `notify.machine_label` | `null` | The machine half of the `<repo>@<machine>` line every text starts with (issue #493). `null` uses the host's short hostname; set a short name such as `"mini"` or `"laptop"` to tell two machines' loops apart on a lock screen. |
| `notify.quiet_hours` | `null` | Parks, bounces and durable questions text you when they happen, at any hour; your phone's Do Not Disturb is the intended night filter. Opt-in: set a window such as `{"start": "21:00", "end": "08:00"}` (Mac-local, `end` exclusive, may wrap midnight) and routine owner-**decision** pages (a park, a bounce, a durable question) raised inside it are **batched to the morning report** instead of pushed. Systemic-stop alerts (runner/auth dead, whole queue stalled) and the merge-freeze notice push even inside a window. A repo adopted before this default changed may have `adopt`'s old 21:00–08:00 window written into its config; set the key to `null` to turn it off. |
| `report_time` | `"08:45"` | When the morning report is generated (Mac-local time). It is written every day but texted only when it has news — a merge, a park or needs-owner hand-back, a bounce, a question, a regeneration, a wander, debugger or resurrection activity, a triage verdict, a held queue, or a hold or freeze past its age threshold (issue #495). There is no heartbeat text: every text the loop sends is journaled, and the report's **Notify channel** line and the dashboard's truth strip show how long ago one last reached your phone, saying plainly when none has in more than a week. |
| `runner_home` | `"pane"` | Where the **runner's own process** lives (issue #306): `pane` — a visible multiplexer tab you opened, whose pane is the launch anchor every worker session is born in — or `login-item`, a plain `gui/$UID` LaunchAgent talking to the session host's server. It selects the boot preflight, how a Restart request is executed, and how the watchdog restarts a dead runner. Leave it `pane` unless you have migrated to a session host whose spawn needs no anchor; `login-item` then needs one setup step, `superlooper runner-home --repo <path> --install --load`, which records where `gh` and `git` actually resolve on your machine (launchd's own PATH does not include them). See `skills/superlooper/docs/RUNNER-HOME.md`. |
| `triage.enabled` | `false` | Master switch for the **triage flight** (issue #448) — the `t<N>` session class that keeps the UNAPPROVED queue tidy under the owner's standing rule (`plugin/skills/superlooper/references/triage-standing-rule.md`): format/label fixes, duplicate merges, and closes as overtaken or as a nit-to-the-ledger. It never touches an approved (`agent-ready`) issue, never applies an approval label, and holds no session-host token. Ships **off**; turn it on only once the rest of the triage wave has landed in your engine. |
| `triage.home` | `"checkout"` | Where a triage flight runs. `checkout` opens the repo's **real checkout**, so the flight sees what an orchestrator sees — the gitignored working files a fresh worktree by definition cannot show. Its working tree stays read-only to the flight (its only writes are GitHub and its own state home). Set `worktree` if your gitignored overlay is sensitive, accepting that overlay-aware triage is lost there; that home is a **detached** checkout of `dev_branch`, since the flight creates no branch. Each flight gets its own checkout under `<state home>/worktrees/`, and the runner reclaims one once that flight's session has ended (issue #463) — ungated, unlike `cleanup_parked_worktrees`, because a flight's checkout holds no work to look at: it commits nothing and pushes nothing. |
