...
| `auto_close_merged_windows` | `true` | Auto-close a lane's cmux window (and, ordered by #149, reclaim its worktree) once it has **successfully merged and landed** — owner ruling 2026-07-16 (#168). This is the ONLY automatic window close. Set `false` to keep merged windows/worktrees for inspection (the pre-#149 "nothing auto-closed" posture); `superlooper tidy` then remains your explicit word to close them. |
