...
def _fleet(**over):
    """A machine that declares its fleet fenced — the environment the launcher reads it from."""
    env = {"SL_FLEET_FENCE": "required", "HERDR_SOCKET_PATH": _SOCK}
    env.update(over)
    return env


def test_an_open_socket_refuses_a_worker_launch(tmp_path):
    ...
    spec = _spec(tmp_path, host_env=_fleet())
...
def test_a_fenced_socket_permits_a_worker_launch(tmp_path):
...
def test_an_unreachable_socket_refuses_a_worker_launch(tmp_path):
...
def test_an_unfenced_dev_host_stays_usable(tmp_path):
    ...
    spec = _spec(tmp_path, host_env={"HERDR_SOCKET_PATH": _SOCK})
    result, edges, host = _run(spec, edges=edges)
    assert result.rc == launch.OK, result.stderr
    assert [s["name"] for s in host.spawned] == ["i308"]
...
def test_an_explicit_off_is_the_same_as_saying_nothing(tmp_path):
    edges = FakeEdges(fence=session_host.OPEN)
    spec = _spec(tmp_path, host_env=_fleet(SL_FLEET_FENCE="off"))
    result, _edges, host = _run(spec, edges=edges)
    assert result.rc == launch.OK, result.stderr
    assert host.spawned != []


def test_a_switch_value_this_engine_cannot_read_arms_the_gate_and_names_itself(tmp_path):
    ...
    edges = FakeEdges(fence=session_host.OPEN)
    spec = _spec(tmp_path, host_env=_fleet(SL_FLEET_FENCE="requried"))
    result, _edges, host = _run(spec, edges=edges)
    assert result.rc == launch.FENCE_DOWN
...
def test_the_gate_cannot_be_disarmed_by_the_attended_flag(tmp_path):
    ...
    edges = FakeEdges(fence=session_host.OPEN)
    spec = _spec(tmp_path, host_env=_fleet(), attended=True)
    result, _edges, host = _run(spec, edges=edges)
    assert result.rc == launch.FENCE_DOWN
    assert host.spawned == []


def test_a_debugger_launch_is_never_gated_by_the_fence(tmp_path):
...
def test_the_refusal_costs_no_worktree_no_pretrust_and_no_pane(tmp_path):
...
def test_the_probe_asks_the_socket_the_spawn_itself_would_use(tmp_path):
    ...
    edges = FakeEdges(fence=session_host.FENCED)
    env = {"SL_FLEET_FENCE": "required", "HOME": "/Users/x", "HERDR_SESSION": "fleet"}
    spec = _spec(tmp_path, host_env=env)
    result, edges, _host = _run(spec, edges=edges)
    assert result.rc == launch.OK, result.stderr
    assert edges.fence_asked == [session_host.control_socket_path(env)]
    assert edges.fence_asked == ["/Users/x/.config/herdr/sessions/fleet/herdr.sock"]


def test_a_machine_whose_socket_cannot_be_resolved_refuses_rather_than_guessing(tmp_path):
    ...
    edges = FakeEdges(fence=session_host.FENCED)
    spec = _spec(tmp_path, host_env={"SL_FLEET_FENCE": "required"})   # no HOME, no override
    result, edges, host = _run(spec, edges=edges)
    assert result.rc == launch.FENCE_DOWN
...
def test_the_probe_verdict_is_journaled_on_every_worker_launch(tmp_path):
...
def test_an_unfenced_machine_that_launches_anyway_still_leaves_a_record(tmp_path):
...
def test_a_refusal_is_journaled_as_a_refusal(tmp_path):
    edges = FakeEdges(fence=session_host.OPEN)
    spec = _spec(tmp_path, host_env=_fleet())
...
def test_an_unjournalable_verdict_never_decides_the_launch(tmp_path):
